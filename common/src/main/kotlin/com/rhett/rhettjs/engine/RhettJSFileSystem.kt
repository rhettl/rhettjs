...
class RhettJSFileSystem(
    ...
    companion object {
        private val BUILT_IN_MODULES = setOf("World", "Structure", "StructureNbt", "LargeStructureNbt", "WorldgenStructure", "Store", "NBT", "Server", "Commands", "Runtime", "Script")
    ...
    private fun generateBuiltInModule(name: String): String {
        ...
        // Special handling for legacy "Structure" import (combines StructureNbt and LargeStructureNbt)
        if (name == "Structure") {
            val moduleCode = """
                // Built-in module: Structure (combined StructureNbt + LargeStructureNbt)
                // Generated by RhettJSFileSystem
                const structureNbt = globalThis.__builtin_StructureNbt;
                const largeStructureNbt = globalThis.__builtin_LargeStructureNbt;
                if (!structureNbt) {
                    throw new Error('Built-in module "StructureNbt" not found on globalThis');
                }
                if (!largeStructureNbt) {
                    throw new Error('Built-in module "LargeStructureNbt" not found on globalThis');
                }
                // Combine both APIs into a single object using Object.assign
                const combined = Object.assign({}, structureNbt, largeStructureNbt);
                export default combined;
            """.trimIndent()
            ConfigManager.debug("Generated virtual module for '$name' (combined StructureNbt + LargeStructureNbt):\n$moduleCode")
            return moduleCode
        }
